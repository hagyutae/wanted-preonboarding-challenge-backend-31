...
export const productController = {
  ...
  async getProducts(
    req: Request,
    res: Response,
  ): Promise<Response<PaginatedResponse<ProductListItemResponse>>> {
    try {
      // 페이지네이션 파라미터 파싱
  ...
  async getProductById(
    req: Request,
    res: Response,
  ): Promise<Response<ApiResponse<ProductDetailResponse>>> {
    try {
      const productId = parseInt(req.params.id);
      const product = await productService.getProductById(productId);

      return res.json(createSuccessResponse(product, '상품 상세 정보를 성공적으로 조회했습니다.'));
    } catch (error) {
      const errorMessage =
        error instanceof Error
          ? error.message
          : '상품 상세 정보를 가져오는 중 오류가 발생했습니다.';
  ...
  async createProduct(
    req: Request,
    res: Response,
  ): Promise<Response<ApiResponse<ProductCreateResponse>>> {
    try {
      const {
        name,
        slug,
      ...
      const productData: Prisma.ProductCreateInput = {
        ...
        categories:
          categories && categories.length > 0
            ? {
                create: categories.map((cat: { category_id: number; is_primary?: boolean }) => ({
      ...
      return res
        .status(201)
        .json(createSuccessResponse(response, '상품이 성공적으로 등록되었습니다.'));
    } catch (error) {
  ...
  async updateProduct(
    req: Request,
    res: Response,
  ): Promise<Response<ApiResponse<ProductUpdateResponse>>> {
    try {
      const productId = parseInt(req.params.id);
      const {
        name,
      ...
      return res.json(createSuccessResponse(response, '상품이 성공적으로 수정되었습니다.'));
    } catch (error) {
      const errorMessage =
  ...
  async deleteProduct(req: Request, res: Response): Promise<Response<ApiResponse<null>>> {
    try {
      ...
      return res.json(createSuccessResponse(null, '상품이 성공적으로 삭제되었습니다.'));
    } catch (error) {
      const errorMessage =
  ...
  async addProductOption(
    req: Request,
    res: Response,
  ): Promise<Response<ApiResponse<ProductOptionModifyResponse>>> {
    try {
      const productId = parseInt(req.params.id);
      const { option_group_id, name, additional_price, sku, stock, display_order } = req.body;
      ...
      return res
        .status(201)
        .json(createSuccessResponse(response, '상품 옵션이 성공적으로 추가되었습니다.'));
    } catch (error) {
      const errorMessage =
        error instanceof Error ? error.message : '상품 옵션 추가 중 오류가 발생했습니다.';
      const status = errorMessage === '상품을 찾을 수 없습니다.' ? 404 : 500;
      const errorCode = status === 404 ? ErrorCode.RESOURCE_NOT_FOUND : ErrorCode.INTERNAL_ERROR;
  ...
  async updateProductOption(
    ...
    try {
      ...
      const { name, additional_price, sku, stock, display_order } = req.body;
      ...
      return res.json(createSuccessResponse(response, '상품 옵션이 성공적으로 수정되었습니다.'));
    } catch (error) {
      const errorMessage =
        error instanceof Error ? error.message : '상품 옵션 수정 중 오류가 발생했습니다.';
      const status = errorMessage === '상품을 찾을 수 없습니다.' ? 404 : 500;
      const errorCode = status === 404 ? ErrorCode.RESOURCE_NOT_FOUND : ErrorCode.INTERNAL_ERROR;
  ...
  async deleteProductOption(req: Request, res: Response): Promise<Response<ApiResponse<null>>> {
    try {
      ...
      return res.json(createSuccessResponse(null, '상품 옵션이 성공적으로 삭제되었습니다.'));
    } catch (error) {
      const errorMessage =
        error instanceof Error ? error.message : '상품 옵션 삭제 중 오류가 발생했습니다.';
      const status = errorMessage === '상품을 찾을 수 없습니다.' ? 404 : 500;
      const errorCode = status === 404 ? ErrorCode.RESOURCE_NOT_FOUND : ErrorCode.INTERNAL_ERROR;
  ...
  async addProductImage(
    req: Request,
    res: Response,
  ): Promise<Response<ApiResponse<ProductImageAddResponse>>> {
    try {
      const productId = parseInt(req.params.id);
      const { url, alt_text, is_primary, display_order, option_id } = req.body;
      ...
      return res
        .status(201)
        .json(createSuccessResponse(response, '상품 이미지가 성공적으로 추가되었습니다.'));
    } catch (error) {
      const errorMessage =
        error instanceof Error ? error.message : '상품 이미지 추가 중 오류가 발생했습니다.';
      const status = errorMessage === '상품을 찾을 수 없습니다.' ? 404 : 500;
      const errorCode = status === 404 ? ErrorCode.RESOURCE_NOT_FOUND : ErrorCode.INTERNAL_ERROR;
  ...
  async getProductReviews(
    req: Request,
    res: Response,
  ): Promise<Response<ApiResponse<ReviewListResponse>>> {
    try {
      const productId = parseInt(req.params.id);
      const paginationParams = parsePaginationParams(req.query);
      ...
      return res.json(createSuccessResponse(response, '상품 리뷰를 성공적으로 조회했습니다.'));
    } catch (error) {
      const errorMessage =
        error instanceof Error ? error.message : '상품 리뷰 조회 중 오류가 발생했습니다.';
      const status = errorMessage === '상품을 찾을 수 없습니다.' ? 404 : 500;
      const errorCode = status === 404 ? ErrorCode.RESOURCE_NOT_FOUND : ErrorCode.INTERNAL_ERROR;
  ...
  async createProductReview(
    req: Request,
    res: Response,
  ): Promise<Response<ApiResponse<ReviewResponse>>> {
    try {
      const productId = parseInt(req.params.id);
      ...
      return res
        .status(201)
        .json(createSuccessResponse(review, '리뷰가 성공적으로 등록되었습니다.'));
    } catch (error) {
      const errorMessage =
        error instanceof Error ? error.message : '상품 리뷰 작성 중 오류가 발생했습니다.';
      const status = errorMessage === '상품을 찾을 수 없습니다.' ? 404 : 500;
      const errorCode = status === 404 ? ErrorCode.RESOURCE_NOT_FOUND : ErrorCode.INTERNAL_ERROR;
