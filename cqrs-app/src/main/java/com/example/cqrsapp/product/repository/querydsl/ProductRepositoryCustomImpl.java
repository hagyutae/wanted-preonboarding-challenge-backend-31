...
import com.example.cqrsapp.product.repository.ProductRepositoryCustom;
...
public class ProductRepositoryCustomImpl implements ProductRepositoryCustom {
    ...
    @Override
    public Page<ProductSummaryItem> findAll(SearchParm searchParm, Pageable pageable) {
        List<ProductSummaryItem> result = queryFactory
                ...
                .where(isPrimaryImage(),
                        productStatusEq(searchParm.getStatus()),
                        minPriceGoe(searchParm.getMinPrice()),
                        mixPriceLoe(searchParm.getMaxPrice()),
                        categoryIn(searchParm.getCategory()),
                        sellerIdEq(searchParm.getSeller()),
                        brandNameEp(searchParm.getBrand()),
                        inStock(searchParm.getInStock()),
                        nameContains(searchParm.getSearch())
                )
        ...
        return PageableExecutionUtils.getPage(result, pageable, () -> productTotalCount(searchParm));
    }

    private Long productTotalCount(SearchParm searchParm) {
        return queryFactory
    ...
    private static BooleanExpression minPriceGoe(Long minPrice) {
        return minPrice != null ? productPrice.salePrice.goe(minPrice) : null;
    }

    private static BooleanExpression mixPriceLoe(Long maxPrice) {
        return maxPrice != null ? productPrice.salePrice.loe(maxPrice) : null;
    }

    private static BooleanExpression sellerIdEq(Long sellerId) {
        return sellerId != null ? seller.id.eq(sellerId) : null;
    }

    private static BooleanExpression brandNameEp(Long brandId) {
        return brandId != null ? brand.id.eq(brandId) : null;
    }

    private static BooleanExpression inStock(Boolean inStock) {
        return inStock != null && inStock ? stockCount().goe(0L) : null;
    }

    private static BooleanExpression nameContains(String searchText) {
        return StringUtils.hasText(searchText) ? product.name.contains(searchText) : null;
    }
